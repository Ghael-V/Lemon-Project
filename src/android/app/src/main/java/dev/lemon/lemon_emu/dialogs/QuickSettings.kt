...
import dev.lemon.lemon_emu.features.settings.model.AbstractSetting
import dev.lemon.lemon_emu.features.settings.model.AbstractShortSetting
...
class QuickSettings(val emulationFragment: EmulationFragment) {
    ...
    fun addIntSetting(
        ...
        titleView.text = LemonApplication.appContext.getString(name)
    ...
    fun addPostProcessing(container: ViewGroup, onStructureChanged: () -> Unit) {
        ...
        if (preset.isNotEmpty()) {
            ...
            if (summary.isEmpty()) {
                summary =
                    LemonApplication.appContext.getString(R.string.post_processing_preset_locked)
            }
            if (NativePostProcessing.isPresetModified()) {
                summary = summary + "\n" +
                    LemonApplication.appContext.getString(R.string.post_processing_preset_modified)
            }

...
import dev.lemon.lemon_emu.fragments.CoreErrorDialogFragment
...
import dev.lemon.lemon_emu.model.InstallResult
import dev.lemon.lemon_emu.model.Patch
import dev.lemon.lemon_emu.model.GameVerificationResult
import dev.lemon.lemon_emu.network.NetPlayManager
import dev.lemon.lemon_emu.applets.web.WebBrowser

/**
 * Class which contains methods that interact
 * with the native side of the Lemon code.
 */
object NativeLibrary {
    ...
    init {
        try {
            System.loadLibrary("lemon-android")
            SDL.setupJNI()
            initJvm()
    ...
    fun openContentUri(path: String?, openmode: String?): Int {
        return if (DocumentsTree.isNativePath(path!!)) {
            LemonApplication.documentsTree!!.openContentUri(path, openmode)
        } else {
    ...
    fun getSize(path: String?): Long {
        return if (DocumentsTree.isNativePath(path!!)) {
            LemonApplication.documentsTree!!.getFileSize(path)
        } else {
    ...
    fun exists(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            LemonApplication.documentsTree!!.exists(path)
        } else {
    ...
    fun isDirectory(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            LemonApplication.documentsTree!!.isDirectory(path)
        } else {
    ...
    fun getParentDirectory(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            LemonApplication.documentsTree!!.getParentDirectory(path)
        } else {
            path
        }
    ...
    fun getFilename(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            LemonApplication.documentsTree!!.getFilename(path)
        } else {
    ...
    /**
     * Logs the Lemon version, Android version and, CPU.
     */

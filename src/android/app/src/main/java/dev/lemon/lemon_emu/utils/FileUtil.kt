...
object FileUtil {
    ...
    private val context get() = LemonApplication.appContext
    ...
    fun getFilename(uri: Uri): String {
        ...
        val resolver = LemonApplication.appContext.contentResolver
        val columns = arrayOf(
            DocumentsContract.Document.COLUMN_DISPLAY_NAME
        )
        var filename = ""
    ...
    fun DocumentFile.copyFilesTo(
        ...
        this.listFiles().forEach {
            ...
            if (it.isDirectory) {
                newFile.mkdirs()
                DocumentFile.fromTreeUri(LemonApplication.appContext, it.uri)?.copyFilesTo(newFile)
            } else {
                val inputStream =
                    LemonApplication.appContext.contentResolver.openInputStream(it.uri)
    ...
    fun DocumentFile.inputStream(): InputStream =
        LemonApplication.appContext.contentResolver.openInputStream(uri)!!

    fun DocumentFile.outputStream(): OutputStream =
        LemonApplication.appContext.contentResolver.openOutputStream(uri)!!

    fun Uri.inputStream(): InputStream =
        LemonApplication.appContext.contentResolver.openInputStream(this)!!

    fun Uri.outputStream(): OutputStream =
        LemonApplication.appContext.contentResolver.openOutputStream(this)!!

    fun Uri.asDocumentFile(): DocumentFile? =
        DocumentFile.fromSingleUri(LemonApplication.appContext, this)
}
